...
void call_cb_fn(const v8::FunctionCallbackInfo<v8::Value>& info){
    ...
    irods::foreach2(ps0, boost::irange(2,info.Length()), [&](std::string& a, int b) {
        ps.emplace_back(&a);
    ...
    if(!err.ok()) {
        ...
        irods::foreach2(ps, boost::irange(0,info.Length()-2), [&](boost::any& a, int b) {
...
irods::error exec_rule(irods::default_re_ctx&, std::string fn, std::list<boost::any>& ps, irods::callback cb) {
    ...
    if (js_res->IsArray()) {
        ...
        irods::foreach2 (ps, boost::irange(0, static_cast<int>(arr->Length())), [&](boost::any& a, int b) {
